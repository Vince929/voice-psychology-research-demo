...
- **结构化 LLM 调用**：DeepSeek 强制 JSON 输出（`response_format: json_object`），服务端校验必填字段、分数范围、情感词表与禁用词。
- **双曲线声学图表**：前端 React Native 绘制基线对齐的「音量 + 音调」时序曲线，标注动态范围与有声帧统计。
- **多租户匿名采集**：匿名编号隔离受访者，按编号聚合多条录音，所有元数据仅用于记录与质量解释。
- **幂等设计**：上传 `idempotency_key` + 分析任务唯一约束，重复提交不会产生重复记录。
